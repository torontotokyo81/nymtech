...
mod tests {
    ...
    fn blind_sign_request_bytes_roundtrip() {
        ...
        let lambda = prepare_blind_sign(
            ...
            .unwrap();
        println!("ElGamal public key gamma: {:?}", elgamal_keypair.public_key().to_bytes().len());
        println!("BlindSign Request (commitment, commitment hash, encrypted priv attributes, proof) : {:?}", lambda.to_bytes().len());
